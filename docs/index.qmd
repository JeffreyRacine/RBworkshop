...
- Consider two variables, one of which is numeric (`lwage`) and the other of which is ordered (`numdep`)

- `lwage` is the logarithm of an individual's average hourly earnings, and `numdep` is the individual's number of dependants $\{0,1,\dots,6\}$
...
- We estimate the joint density and plot it in @fig-wage1mixeddensity

## Example - Mixed Data Density

- Below is the R code to plot the joint density (we use the `plot3D` package and reformat the data to render this plot)
